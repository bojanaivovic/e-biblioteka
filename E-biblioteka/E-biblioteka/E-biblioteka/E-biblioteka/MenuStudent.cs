...
//BRISANJE POZAJMICE---> NAKON 30 dana SE BRISE, vraceno nije da, jer student nije vratio!!!
            con.Open();
            string query2 = "UPDATE pozajmica set  isteklo='Da' where datediff(now(),datum_uzimanja)>30 and korisnik_ID='" + korisnik_ID + "' and vraceno='Ne'";
            MySqlCommand cmd2 = new MySqlCommand(query2, con);
            cmd2.ExecuteNonQuery();
            con.Close();
...
//----------------------------------------obavestenja za studenta----------------------
        ...
        {
            con.Open();
            string query = "SELECT pozajmica_ID, knjiga_ID, naziv_dela, datum_uzimanja from pozajmica inner join dela on knjiga_ID=delo_ID where korisnik_ID='" + korisnik_ID + "' and isteklo='Da' and seen_student!=1 ";
